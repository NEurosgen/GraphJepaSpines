...
class GraphPruning(torch.nn.Module):
    '''
    Prune existing graph edges based on maximum spatial distance (radius).
    Retains original edge_attr for the surviving edges.
    '''
    ...
    def forward(self, data):
        ...
        dist = torch.linalg.norm(data.pos[row] - data.pos[col], dim=-1)
        mask = dist <= self.r
...
class MaskData(torch.nn.Module):
    '''
    Waring! This class return TWO graphs, context and target (In JEPA notations)
    '''
    def __init__(self, mask_ratio: float):
        super().__init__()
        self.mask_ratio = mask_ratio

    def _get_random_node_mask(self, data) -> torch.Tensor:
    ...
    def _split_data_by_mask(self, data, mask):
        ...
        def build_subgraph(subset):
            ...
            for key, item in data:
                if data.is_node_attr(key) and isinstance(item, torch.Tensor):
